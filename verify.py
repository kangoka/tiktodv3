...
from app import normalize_tiktok_url
from bot import RATE_LIMIT_RE, Bot
...
def check_captcha_text_validation():
    ...
    with patch("captcha_solver.pytesseract.image_to_string", return_value="Loading"):
        with patch("captcha_solver.Image.open") as image_open:
...
def check_rate_limit_handling():
    ...
    bot._sleep_interruptible = lambda seconds: None  # type: ignore[method-assign]
    with patch(
        "bot.random.randint", return_value=RATE_LIMIT_MIN_BACKOFF_SECONDS
    ) as randint:
